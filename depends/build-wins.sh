...
if [ ! -f $CC ]; then
   sudo apt-get install build-essential libtool autotools-dev automake cmake pkg-config bsdmainutils curl g++-mingw-w64-x86-64 mingw-w64-x86-64-dev g++-mingw-w64-i686 mingw-w64-i686-dev openjdk-8-jdk -y
fi
...
if [ -z "$JAVA_HOME" ]; then
   ...
   echo "JAVA_HOME env var set to $JAVA_HOME!"
fi
...
./configure --prefix=$PWD/depends/$PLATFORM --host=$PLATFORM --disable-shared --enable-reduce-exports CPPFLAGS="-DMINIUPNP_STATICLIB" CFLAGS="-std=c99 -I$JAVA_HOME/include -I$JAVA_HOME/include/linux" LDFLAGS="-static -static-libgcc -Wl,-Bstatic -lstdc++"
